...
def dashbord(request):
    NoOfSociety = SocietyDeatils.objects.count()
    print(NoOfSociety)

    activeSociety = SocietyDeatils.objects.filter(is_active=1).count()
    print(activeSociety)

    deactiveSociety = SocietyDeatils.objects.filter(is_active=0).count()
    print(deactiveSociety)

    # topData = SocietyDeatils.objects.filter()
...
def society_list(request):
    if request.method == 'POST':
        ...
        society_list = SocietyDeatils.objects.all()

        if contact_name != "NULL":
            society_list = SocietyDeatils.objects.filter(contact_name=contact_name)
        if society_name != "NULL":
            society_list = SocietyDeatils.objects.filter(society_name=society_name)
        if city != "NULL":
            society_list = SocietyDeatils.objects.filter(city=city)
        print(society_list)
...
def statusChange(request, id):
    User_Society_detail = SocietyDeatils.objects.get(id=id)
...
def editSocietyList(request, id):
    print("edit ------------")
    society_list = SocietyDeatils.objects.get(id=id)
